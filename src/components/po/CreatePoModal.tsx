...
import { createPurchaseOrder } from '@/actions/po';

export default function CreatePoModal({ products }: { products: any[] }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50/50">
              <h2 className="text-lg font-semibold text-gray-900">Form Purchase Order Baru</h2>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-5 max-h-[80vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5 focus-within:text-blue-600">
                  <label className="text-sm font-medium text-gray-700">Nomor PO *</label>
                  <input required name="po_no" placeholder="Contoh: PO-2026-001" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div className="space-y-1.5 focus-within:text-blue-600">
                  <label className="text-sm font-medium text-gray-700">Kode Supplier *</label>
                  <input required name="supplier_id" placeholder="Cth: SUP-001" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>

              <div className="space-y-2">
                <div className="flex justify-between items-center border-b pb-2">
                  <label className="text-sm font-bold text-gray-700">Daftar Barang Pesanan</label>
                ...
                {items.map((item, idx) => (
                  <div key={idx} className="flex gap-2">
                     ...
                     />
                     <input 
                       required type="number" placeholder="Harga Beli (Rp)" 
              ...
              <div className="space-y-1.5">
                 <label className="text-sm font-medium text-gray-700">Catatan Khusus</label>
              ...
              <div className="pt-4 flex justify-end gap-3 mt-2 border-t border-gray-100">
                ...
                <button disabled={loading} type="submit" className="px-5 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 shadow-sm disabled:opacity-50 transition">
                  {loading ? 'Menyimpan...' : 'Kirim PO & Simpan'}
                </button>
              </div>
            </form>
          </div>
